...
# ────────────────────────── utilities ────────────────────────────────────
# _sha256 removed in favor of inline chunked hashing
...
def init_session():
    ...
    if st.session_state.vector_store is None:
        st.session_state.vector_store = _new_store()
...
def _process(fobj, fn):
    dn = display_name(fn)
    tp = "PDF" if fn.lower().endswith(".pdf") else "IMAGE"
    txt = _extract_pdf(fobj, dn) if tp == "PDF" else _extract_img(fobj, dn)
    return _split(txt, dn, tp), tp
...
# ───────────────── ingestion ────────────────────────────────────────────
def add_to_db(fobj, fn, h):
    if h in st.session_state.processed_hashes:
        return
    chunks, tp = _process(fobj, fn)
    if not chunks:
        st.warning(f"Skipped empty/unsupported: {display_name(fn)}")
        return
    dn = display_name(fn)
    st.session_state.vector_store.add_texts(chunks, metadatas=[{"source": dn, "type": tp}] * len(chunks))
    st.session_state.processed_hashes.add(h)
    st.session_state.source_files.add(dn.lower())
...
def build_ctx(q, k=5):
    ...
    docs = st.session_state.vector_store.similarity_search(q, k=20)
    if names or types:
...
def show_files():
    ...
    for fn in sorted(st.session_state.source_files):
        ft = "PDF" if fn.lower().endswith(".pdf") else "IMAGE"
        st.caption(f"• {fn} ({ft})")
